...
def gen_sample(plate, width, height):
    name, label = gen_rand()
    img = plate.generate(name)
...
def gen_batch(batch_size, output_path):
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    label_store = []
    for i in range(batch_size):
        print('create num:' + str(i))
        label, name, img = gen_sample(gen_plate, 120, 30)
        label_store.append(label)
        filename = os.path.join(output_path, str(i).zfill(4) + ".jpg")
...
bg_dir = './backgrounds'
gen_plate = GenPlate(font_ch, font_en, bg_dir)
gen_batch(batch_size=batchSize, output_path=path)
